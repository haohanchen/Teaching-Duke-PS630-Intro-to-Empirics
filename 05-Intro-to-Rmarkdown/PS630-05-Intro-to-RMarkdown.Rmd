---
title: "`Rmarkdown` for Data Analysis: Beyond the Basics"
author:
...
This is a refresher of some common `Rmarkdown` operations. I demonstrate it by writing a mini data analysis report on a toy dataset.
...
```{r setup}
  ...
  knitr::opts_chunk$set(echo = TRUE, 
                        ...
                        fig.path = "figures/", 
                        size = "tiny")
  # size option: small = usual size; 
    # Also: "Huge", "huge", "LARGE", "Large", "large", 
...
## Correlcation Matrix
```{r cormat, fig.width=8, fig.height=6, fig.align='center', out.width="50%", fig.pos="h!", fig.cap="\\label{fig:corrmat}Correlation Matrix"}
#---------------------------
# Correlation Matrix
#---------------------------
...
#---------------------------
  ...
  fit_models <- function(d){
    m <- list()
    m[["Baseline"]] <- lm(force ~ unemp + gnp, data = d)
    m[["Population"]] <- lm(force ~ unemp + gnp + pop, data = d)
    m[["Quad Population"]] <- lm(force ~ unemp + gnp + I(gnp^2) + pop, data = d, 
                                  family = gaussian)
    m[["Year"]] <- lm(force ~ unemp + gnp + I(gnp^2) + pop + yr, data = d)
    m
  }
...
- The `Rmarkdown` way
    - Bla bla bla [@johnston2014ideology].
    - @beramendi2008democracy argue that... 
    - Existing studies find evidence that bla bla bla [see @stegmueller2013many; @bell2015explaining for details]
